...
pub async fn login(
    State(ctx): State<AppContext>,
    jar: CookieJar,
    // Use Form instead of Json to support standard HTML forms
    ...
    // Redirect to the lobby instead of returning JSON
}
...
pub async fn register_view(
    ...
    format::render().view(&v.0, "auth/register.html", json!({}))
}

pub async fn login_view(
    v: ViewEngine<TeraView>, // This extracts the template engine specifically
) -> Result<Response> {
    format::render().view(&v.0, "auth/login.html", json!({}))
}
pub fn routes() -> Routes {
    Routes::new()
        ...
        .add("/login_view", get(login_view))
        .add("/register_view", get(register_view))
}
